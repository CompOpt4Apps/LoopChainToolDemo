 ...
 * \file CHTmsg.cpp
 *
 * \brief Member functions for CHTmsg
 *
 *//*+*************************************************************************/
...
{
  CHT_assert(m_varNameSize > m_varNameTerm.size());
...
//
void
CHT::Msg::setLineSize(const size_t a_lineSize)
{
  CHT_assert(a_lineSize > 0);
...
//
void
CHT::Msg::setOutputStream(std::ostream& a_out)
{
  m_out = &a_out;
}
...
//
void
CHT::Msg::setErrorHeader(const char* a_header)
{
...
//
void
CHT::Msg::setWarnHeader(const char* a_header)
{
...
//
void
CHT::Msg::setHeader1(const char* a_header)
{
  m_header1 = a_header;
}
...
//
void
CHT::Msg::setHeader2(const char* a_header)
{
  m_header2 = a_header;
}
...
//
void
CHT::Msg::setBodyHeader(const char* a_header)
{
...
//
void
CHT::Msg::setVerbosityHeader(const char* a_header, const int a_idxVerb)
{
  CHT_assert(a_idxVerb >= 0 && a_idxVerb <= 4);
...
void
CHT::Msg::setDefaultPrec(const int a_prec)
{
...
/** This should have the same value as CHTparam::g_verbosity.
 *  CHT::Msg does not use it directly to keep this low-level class
 *  separate.
...
void
CHT::Msg::setVerbosity(const int a_verbosity)
{
...
//--With header
   ...
   std::cout << "\nWith header:\n\n";
   CHT::linePrint(text, std::cout, "--->");

//--Change line size

   std::cout << "\nChange line size to 40 and indent after first line:\n\n";
   CHT::linePrint(text, std::cout, "->", -1, 40);

//--Get real fancy

   std::cout << "\nGet real fancy:\n\n";
   
   int j = CHT::linePrint(text, std::cout, "---\\ ", 1, 40);
   j += CHT::linePrint(text+j, std::cout, "----\\", 1, 40);
   j += CHT::linePrint(text+j, std::cout, "-----\\", 1, 40);
   CHT::linePrint(text+j, std::cout, "------\\", 0, 40);
}                                                
 *                                                                  \endverbatim
 *//*=========================================================================*/

size_t
CHT::linePrint(const char*   a_message,
               std::ostream& a_out,
               const char*   a_header,
               int           a_numLines,
               size_t        a_lLineSize)
...
//--Initialize input
  ...
  if (a_lLineSize <= 0)
    {
      a_lLineSize = CHT::defaultLineSize;
    }
